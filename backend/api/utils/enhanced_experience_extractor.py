...
class EnhancedExperienceExtractor:
    ...
    def extract_total_experience(self, resume_text: str, fallback_to_pyresparser: bool = True) -> int:
        ...
        try:
            ...
            # Approach 3: Try pytorch parser if enabled and available
            if fallback_to_pyresparser:
                try:
                    from .pytorch_resume_parser import PyTorchResumeParser
                    parser = PyTorchResumeParser()
                    parsed_data = parser._extract_experience(resume_text) if hasattr(parser, '_extract_experience') else {}
