...
// import { Send, Sparkles, RefreshCw, Loader2, Upload } from "lucide-react"
...
//         let generated = response.data.description || ""
...
//         // ✅ Switch to description tab automatically
...
//       // ✅ Move to description tab so user sees it
//       setActiveTab("description")
...
export default function JobCreationForm({ onJobCreated }: JobCreationFormProps) {
  ...
  useEffect(() => {
    if (showForm) {  // ✅ only load when form is visible
      const fetchData = async () => {
        try {
          setIsLoading(true)
          const departmentsResponse = await getDepartments()
  ...
  const generateJobDescription = async () => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        setJobDescription(generated)
        setActiveTab("description") // auto move
        toast({
          title: "Success!",
          description: response.ai_generated 
            ? "Job description generated successfully using AI." 
  ...
  return (
    <div className="p-6 space-y-6">
       <h2 className="text-2xl font-bold text-gray-900">Create New Job Posting</h2>
          <p className="text-gray-600">Define the details and description for your new job.</p>
      {!showForm ? (
        <Button onClick={() => setShowForm(true)}>+ Job</Button>
      ) : (
        <>
            ...
            <TabsContent value="details" className="mt-4">
              <Card>
                ...
                <CardContent className="space-y-4">
                  <div className="space-y-2">
                    <Label htmlFor="title">Job Title</Label>
                    <Input id="title" value={jobDetails.title} onChange={handleDetailChange} placeholder="e.g., Senior Software Engineer"/>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      <Label htmlFor="location">Location</Label>
                      <Input id="location" value={jobDetails.location} onChange={handleDetailChange} placeholder="e.g., Remote, New York, NY"/>
                    </div>
                    <div className="space-y-2">
                      <Label htmlFor="workType">Work Type</Label>
                      <Select value={jobDetails.workType} onValueChange={(val) => handleSelectChange("workType", val)}>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      <Label htmlFor="minSalary">Min Salary</Label>
                      <Input id="minSalary" type="number" value={jobDetails.minSalary} onChange={handleDetailChange} placeholder="80000"/>
                    </div>
                    <div className="space-y-2">
                      <Label htmlFor="maxSalary">Max Salary</Label>
                      <Input id="maxSalary" type="number" value={jobDetails.maxSalary} onChange={handleDetailChange} placeholder="120000"/>
                    </div>
                  </div>

                  <div className="space-y-2">
                    <Label htmlFor="experienceRange">Experience Range</Label>
                    <Select value={jobDetails.experienceRange} onValueChange={(val) => handleSelectChange("experienceRange", val)}>
                      <SelectTrigger id="experienceRange">
                        <SelectValue placeholder="Select experience range" />
                      </SelectTrigger>
                      <SelectContent>
                ...
                <CardContent className="space-y-4">
                  <div className="flex space-x-4 mb-4">
                    <label htmlFor="jd-upload" className="flex-1">
                      ...
                      <input id="jd-upload" type="file" accept=".txt,.pdf,.doc,.docx" onChange={handleJDUpload} className="hidden"/>
                    </label>
                    <Button onClick={generateJobDescription} disabled={isGenerating} className="flex-1">
                      {isGenerating ? (
                        <>
                  ...
                  <div className="space-y-2">
                    <Label>Job Description</Label>
                    <Textarea value={jobDescription} onChange={(e) => setJobDescription(e.target.value)} rows={12} className="text-sm" placeholder="Enter job description or generate with AI..."/>
                  </div>

                  <div className="flex justify-between">
                    <Button variant="outline" onClick={() => setActiveTab("details")}>Previous</Button>
                    <div className="flex space-x-2">
                      ...
                      <Button onClick={handleSubmit} disabled={isSubmitting || isLoading}>
                        {isSubmitting ? (
                          ...
                          <>
                            <Send className="mr-2 h-4 w-4" /> Create Job
                          </>
                        )}
                      </Button>
                    </div>
